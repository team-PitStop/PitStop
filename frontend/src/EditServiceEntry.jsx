// EditServiceEntry.jsx
// US-8: Edit a Service Entry
// This component:
//   1. Loads the existing service entry data from the backend when the page opens
//   2. Pre-fills the form fields with that data
//   3. Validates required fields (date, mileage, cost)
//   4. Sends a PUT request to update the entry when the user clicks Save
//
// URL pattern: /vehicles/:vehicleId/service-entries/:id/edit

import { useState, useEffect } from "react";
import { useParams, useNavigate } from "react-router-dom";
import axios from "axios";

// Same service type list used in ServiceLog.jsx (US-9)
const serviceTypes = [
...
cat > frontend/src/EditServiceEntry.jsx << 'EOF'
// EditServiceEntry.jsx
// US-8: Edit a Service Entry
// This component:
//   1. Loads the existing service entry data from the backend when the page opens
...
function EditServiceEntry() {
  ...
  const handleSubmit = (e) => {
    ...
    const finalServiceType =
      entry.serviceType === "OTHER" ? customType : entry.serviceType;
    ...
    <div style={{ padding: "40px", maxWidth: "520px" }}>
      ...
      <form onSubmit={handleSubmit}>
        ...
        <label style={{ display: "block", marginTop: "10px" }}>
          ...
          />
          {errors.mileage && (
            <span style={{ color: "red" }}>{errors.mileage}</span>
          )}
        </label>
